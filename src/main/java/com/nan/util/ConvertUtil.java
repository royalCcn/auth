...
public class ConvertUtil {
	...
	private String videofolder = Contants.videofolder; 	 //需要被转换格式的视频目录
	private String targetfolder = Contants.targetfolder; 	 //转码后视频保存的目录
	private String imageRealPath = Contants.imageRealPath;//截图的存放目录
	
	//重构构造方法，传入源视频
	public ConvertUtil(String path) {
	 ...
	 * @return
	 */
	public boolean beginConver(String targetExtension, boolean isDelSourseFile, HttpServletRequest request) {
		//初始化路径
		videofolder = FileUtil.getContext(request) + Contants.videofolder; 	
		targetfolder = FileUtil.getContext(request) + Contants.targetfolder; 	
		imageRealPath = FileUtil.getContext(request) + Contants.imageRealPath;
		...
		if (!checkfile(sourceVideoPath)) {
			System.out.println(sourceVideoPath + "========该文件不存在哟!");
			return false;
		}
		...
		if (process(targetExtension,isDelSourseFile)) {
			...
			//删除原视频+临时视频
			if (isDelSourseFile) {
				String aviTempPath = videofolder + filerealname + ".avi";
				deleteFile(aviTempPath);
			}
	...
	private boolean process(String targetExtension, boolean isDelSourseFile) {
		...
		if (type == 0) {
			...
			//如果type为1，将其他文件先转换为avi，再用ffmpeg转换为指定格式
	...
	private int checkContentType() {
        ...
        //先用别的工具（mencoder）转换为avi(ffmpeg能解析的)格式
	 ...
	 * 先用（mencoder）转换为avi(ffmpeg能解析的)格式.再用ffmpeg解析为指定格式
	 * @param type
	 * @return
	 */
	...
	private boolean processVideoFormat(String oldfilepath, String targetExtension, boolean isDelSourceFile) {
		...
		try {
			//多线程处理加快速度-解决rmvb数据丢失、builder名称要相同
			ProcessBuilder builder = new ProcessBuilder();
			builder.command(commend);
			Process p = builder.start();   //多线程处理加快速度-解决数据丢失
            ...
            new Thread() {
                public void run() {    
                    ...
                    try {    
                        ...
                        while ((lineB = br.readLine()) != null) {    
                            if (lineB != null){
                            	System.out.println(lineB);    //打印ffmpeg转换过程代码
                            }    
                        }    
                    } catch (IOException e) {    
                        e.printStackTrace();    
            ...
            new Thread() {    
                public void run() {    
                    ...
                    try {    
                        ...
                        while ((lineC = br2.readLine()) != null) {    
                            if (lineC != null){
                            	System.out.println(lineC);    //打印ffmpeg转换过程代码
                            }    
                        }    
                    } catch (IOException e) {
                        e.printStackTrace();  
